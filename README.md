...
| 闹钟 | 1.0.1 | [另见独立仓库](https://github.com/molakesizhanfangguangmang/xiaomusic-plugin-alarm) | 多组闹钟，到点播放指定单曲或播报一句话；每条带「放完之后」（放完就停 / 单曲循环），可勾「只响一次，用完销毁」；只写 `crontab_json`，**需另打后端补丁**，装法见其仓库 |

前两个是单插件包；第三个是工具区本体，装插件之前得先有它。

最后四个**与前三个性质不同**：它们除了插件页，还需要一份后端补丁（要覆盖容器里的 `.py`），
所以插件 zip 与补丁都放在各自的仓库里，本页不放。装之前先读对应仓库的 README。

> 若同时装「刷新歌曲时长」与「歌曲管理」：两者补丁共用同一批 xiaomusic 源文件，
> **安装顺序须先刷新歌曲时长、后歌曲管理**。
>
> 「闹钟」的补丁只碰 `crontab.py`，与上面几个不重叠，先后顺序无所谓。
...
- [`xiaomusic-plugin-orphan-cleaner`](https://github.com/molakesizhanfangguangmang/xiaomusic-plugin-orphan-cleaner)：清理孤儿条目（插件页 + 后端补丁）。
- [`xiaomusic-plugin-music-manager`](https://github.com/molakesizhanfangguangmang/xiaomusic-plugin-music-manager)：歌曲管理（插件页 + 后端补丁）。
- [`xiaomusic-plugin-alarm`](https://github.com/molakesizhanfangguangmang/xiaomusic-plugin-alarm)：闹钟（插件页 + 后端补丁）。
